...
public class studyDAO {
	...
	public void updateSeatInfo() {
		...
		String updateSeatInfo = "update SHOWTODAYSTUDYSEAT set showtodaystudyseat.endtime =(SELECT  TS.endtime FROM( "
				+ "SELECT ROW_NUMBER() OVER(PARTITION BY todaytotalSeat.seatNum ORDER BY todaytotalSeat.todate DESC, todaytotalSeat.ENDTIME desc) AS RNUM,"
				+ " todaytotalSeat.* FROM todaytotalSeat ) TS WHERE RNUM = 1 and todate=(to_char(sysdate,'yyyy/mm/dd')) and seatNum = showtodaystudyseat.seatnum)";
